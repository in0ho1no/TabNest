...
※ ViewModel 層は WinUI 非依存の `src/TabNest.ViewModels` プロジェクトに分離し、
   `tests/TabNest.ViewModels.Tests` から単体テストする（テストから TabNest.App は参照しない）。

---

# 開発フロー（ブランチ運用とタスク完了手順）

`dev-Fable` を develop 相当の統合ブランチとして扱う。
各 Task は以下の手順で進める。

```text
1. dev-Fable から TaskX-Y 形式のブランチを作成してチェックアウトする
2. 実装する（単体テスト・結合テストを含む）
3. GUI 要素を追加・変更した場合は、コミット前に GUI 評価を実施する
   （WinAppDriver + Appium による UI テスト、または UI 自動操作での動作確認）
4. dotnet build / dotnet test が成功した時点でコミットする
5. 実装に使用した AI モデルとは別のモデルでコードレビューを行う
6. レビュー指摘を修正してコミットする
7. dev-Fable へマージして Task 完了とする
8. 次の Task も同様にブランチを切って繰り返す
```
