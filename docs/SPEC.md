...
* テキスト入力中（アドレスバー・グループ名編集中）でも動作が破綻しないこと
...
* 各グループは名前を持つ
* グループ追加ボタンまたは Ctrl+G で「グループN」（N は連番）という名前の新規グループを1段追加する
...
* グループ名の右クリックメニュー「お気に入りに保存」で、タブグループを名前付きで保存できる
* 保存内容は「グループ名」と「タブのフォルダパス一覧（並び順含む）」とする
* 保存はスナップショットとする（保存後に元のグループを変更しても、お気に入り側は変わらない）
* 同名のお気に入りが存在する場合は上書きする
...
- ナビゲーションバーのグループ追加ボタンと Ctrl+G を実装する
  （「グループN」を1段追加し、%UserProfile% を開く初期タブを1個作成）
...
- 現在のタブグループをお気に入りとして保存する（同名は上書き・スナップショット）
- お気に入り一覧を取得する
- お気に入りからタブグループを開く
...
```text
- お気に入り保存（新規・同名上書き）の単体テスト
- お気に入りから開く処理の単体テスト（5段上限で拒否されることを含む）
- お気に入り削除の単体テスト
...
- グループ名の右クリックメニューに「お気に入りに保存」を追加する
