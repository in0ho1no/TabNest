...
* **1 回の依頼（1 セッション）で実装するのは 1 Task のみ**とする。
...
```text
main      : リリース用
dev-Fable : 統合ブランチ（develop 相当）
TaskX-Y   : Task 単位の作業ブランチ（例: Task1-1, Task3-7）
```

* `dev-Fable` が存在しない場合は、最初に `main` から作成する（Task 0 リセット後の初回のみ）
* 作業ブランチは必ず `dev-Fable` から `TaskX-Y` 形式で作成する
* `dev-Fable` および `main` への直接コミットは禁止
* Task 完了後に `dev-Fable` へマージする
...
2. dev-Fable から TaskX-Y ブランチを作成してチェックアウトする
3. 実装する
...
9. dev-Fable へマージし、Task 完了として結果を報告する
```
...
* レビュー対象: 当該 Task の差分（`git diff dev-Fable...TaskX-Y`）
...
* 指摘の修正後、修正コミットを行ってから dev-Fable へマージする
